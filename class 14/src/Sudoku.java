...
public class Sudoku {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr[0].length; j++) {
				if (arr[i][j] != 0) {
					makechoice(arr, i * arr.length + j + 1, rownos, colnos, sbnos, arr[i][j]);
				}
			}
		}
	...
	public static void sudoku(int[][] arr, int cellno, int[] rownos, int[] colnos, int[] sbnos) {
		...
		if (arr[rno][cno] == 0) {
			...
			for (int choice = 1; choice <= 9; choice++) {
				if ((no & (1 << choice)) == 0) {
					makechoice(arr, cellno, rownos, colnos, sbnos, choice);
					sudoku(arr, cellno + 1, rownos, colnos, sbnos);
					unmakechoice(arr, cellno, rownos, colnos, sbnos, choice);
				}
			}
		} else {
			sudoku(arr, cellno + 1, rownos, colnos, sbnos);
	...
	public static void makechoice(int[][] arr, int cellno, int[] rownos, int[] colnos, int[] sbnos, int choice) {
		int rno = (cellno - 1) / arr.length;
		int cno = (cellno - 1) % arr.length;
	...
	public static void unmakechoice(int[][] arr, int cellno, int[] rownos, int[] colnos, int[] sbnos, int choice) {
		int rno = (cellno - 1) / arr.length;
		int cno = (cellno - 1) % arr.length;
